...
private:
    ...
    //_id 成员前面的 pragma 告诉 ODB 编译器，以下成员是对象的标识
    // 符。auto说明符指示它是数据库分配的 ID。
#pragma db id auto // 表示 ID 字段将自动生成（通常是数据库中
                   // 的主键）。
    unsigned long _id;
    unsigned short _age;
    std::string _name;
...
// 将 ODB 编译指示组合在一起，并放在类定义之后。它们也可以移动到一个单独
// 的标头中，使原始类完全保持不变
...
// 如果用到了boost库中的接口，则需要使用选项  : --profile boost/date
// time
