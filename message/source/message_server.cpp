...
DEFINE_string(log_file, "message_server.log", "发布模式下，用于指定日志的输出文件");
DEFINE_int32(log_level, 4, "发布模式下，用于指定日志输出等级");
...
DEFINE_string(mysql_pswd, "968745321", "Mysql服务器访问密码");
...
DEFINE_string(mq_msg_queue, "msg_queue_mysql", "持久化消息的发布队列名称");
DEFINE_string(mq_msg_binding_key, "msg.*", "持久化消息的发布队列名称");
