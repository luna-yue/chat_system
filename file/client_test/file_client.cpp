...
ChannelManager::ChannelPtr channel;
...
TEST(put_test, single_file)
{
    ...
    LOG_DEBUG("上传单个文件成功，ID：{}", rsp->file_info().file_id());
}
...
TEST(put_test, multi_file)
{
    // 1. 读取当前目录下的指定文件数据
    std::string body1;
    ASSERT_TRUE(readFile("./base.pb.h", body1));
    std::string body2;
    ASSERT_TRUE(readFile("./file.pb.h", body2));
    ...
    for (int i = 0; i < rsp->file_info_size(); i++)
    {
        ...
        LOG_DEBUG("多文件上传成功：文件ID：{}", multi_file_id[i]);
    }
}
TEST(get_test, multi_file)
{
    ...
    writeFile("file_download_file2", file_data2.file_content());
}
