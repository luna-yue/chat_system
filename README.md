...
```
          ...
          │         │    ┌─────────┘ │ └───┐    │              │
    ┌─────▼──┐ ┌────▼──┐ │  ┌───────▼──┐  │  ┌─▼──────┐  ┌───▼────┐
    │  User  │ │Friend │ │  │Transmite │  │  │ File   │  │ Speech │
    └───┬────┘ └──┬────┘ │  └┬─┬──┬───┘  │  └───┬────┘  └───┬────┘
        │         │      │   │ │  │      │      │           │
        │    ┌────┘      │   │ │  │      │      │           │
   ┌────▼────▼──┐       │   │ │  │      │      │           │
   ...
   └────────────┘       │   │ │         │                  │
        ...
        └────────┘            │  │Msg │ │ESStore│         │
                              ...
                              │                            │
                       ┌──────▼──────┐                     │
                       │ Agent Svc   │  ← Python (LLM+RAG) │
                       └─────────────┘                     │
```

**消息流**：Gateway → Transmite → 查群成员(Redis/MySQL) → 拼消息 → RabbitMQ
→ Message 服务消费落 MySQL, ES Store 消费落 ES

**推送流**：Transmite 返回 target_list → Gateway → RabbitMQ push 队列
→ Gateway consumer 异步推 WebSocket

**8 个 C++ 微服务** + **1 个 Python Agent 引擎**
